	...
	match state:
		alert:
			...
			if last_hit_var == var2 or last_hit_var == null:
				...
				if is_in_range(30000) == true and last_hit_var == var1:
...
func is_in_range(attack_range) -> bool:
	if global_position.distance_to(target.global_position) < attack_range - 0.05 or global_position.distance_to(target.global_position) < attack_range + 0.05:
		return true
	else:
		return false

func _on_enemy_step_one():
	pass

func _on_enemy_step_two():
